 ...
 * the start of the row, giving n + 1 candies to the first person, n + 2 candies
 ...
 * one more than the previous gift). Return an array (of length num_people and
 * sum candies) that represents the final distribution of candies.
 *
 * Example 1:
...
public class N1103_Distribute_Candies_to_People {
    public int[] distributeCandies(int candies, int numPeople) {
        int[] result = new int[numPeople];
        int peopleIndex = 0;
        int candiesIncrement = 0;

        while (candies > 0) {
            if (peopleIndex == numPeople) peopleIndex = 0;

            if (candies > candiesIncrement) candies -= ++candiesIncrement;
            else {
                candiesIncrement = candies;
                candies = 0;
            }

            result[peopleIndex++] += candiesIncrement;
        }

        return result;
    }

    public int[] distributeCandiesLee(int candies, int numPeople) {
        int[] res = new int[numPeople];

        for (int i = 0; candies > 0; ++i) {
            res[i % numPeople] += Math.min(candies, i + 1);
            candies -= i + 1;
        }
